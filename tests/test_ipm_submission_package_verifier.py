...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body><w:p><w:r><w:t>{saxutils.escape(text)}</w:t></w:r></w:p></w:body>
</w:document>
"""
...
def test_ipm_verifier_passes_final_submission_boundary(tmp_path: Path) -> None:
...
def test_ipm_verifier_requires_final_upload_files(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_blocks_stale_rendered_pdf_text(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_blocks_unexpected_manuscript_page_count(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_allows_manuscript_page_count_within_minimum_and_maximum(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_blocks_manuscript_page_count_below_minimum(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_blocks_manuscript_page_count_above_maximum(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_reads_pdfinfo_with_utf8_replacement(tmp_path: Path, monkeypatch) -> None:
    from scripts import verify_ipm_submission_package
    from scripts.verify_ipm_submission_package import check_package
    ...
    monkeypatch.setattr(verify_ipm_submission_package.subprocess, "run", fake_run)
...
def test_ipm_verifier_blocks_legacy_split_pdfs(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
...
def test_ipm_verifier_blocks_kbs_residual_in_highlights(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
    ...
    _write_docx(
        ...
        "\n".join(
            [
                ...
                TITLE,
                "SC-FMA calibrates coarse utility or proxy fidelity into auditable verification-step weights.",
                "Knowledge-Based Systems deployment remains unvalidated.",
            ]
        ),
    )
    ...
    assert not report.ok
    assert any(
        "Highlights.docx contains residual KBS venue wording: Knowledge-Based Systems"
        in error
        for error in report.errors
    )


def test_ipm_verifier_blocks_kbs_residual_in_cover_letter(tmp_path: Path) -> None:
    from scripts.verify_ipm_submission_package import check_package
    ...
    _write_docx(
        ...
        "\n".join(
            [
                ...
                "Haoran Ma",
                "Ningning Wang",
                "We submit this work to Knowledge-Based Systems.",
            ]
        ),
    )
    ...
    assert not report.ok
    assert any(
        "cover_letter.docx contains residual KBS venue wording"
        in error
        for error in report.errors
    )


def test_ipm_verifier_allows_bib_journal_name_kbs(tmp_path: Path) -> None:
    """``.bib`` entries citing the journal ``Knowledge-Based Systems`` are
    legitimate bibliographic references, not venue wording, and must not be
    flagged by the residual-wording check."""
    from scripts.verify_ipm_submission_package import check_package

    package_dir = tmp_path / "final_package"
    _write_final_package(package_dir)
    # Replace references.bib inside latex_source.zip with one that cites the
    # journal ``Knowledge-Based Systems`` (a legitimate bibliographic venue).
    with zipfile.ZipFile(package_dir / "latex_source.zip", "r") as zin:
        entries = {name: zin.read(name) for name in zin.namelist()}
    entries["references.bib"] = (
        b"@article{x, title={x}, journal={Knowledge-Based Systems}}\n"
    )
    with zipfile.ZipFile(package_dir / "latex_source.zip", "w") as zout:
