...
This supplementary package documents the diagnostic evidence boundary for the Information Sciences transfer package: v3.6/v3.8 provide positive process-annotation-distribution evidence and in-distribution frozen PRM baseline-context evidence; GSM8K/HotpotQA task-specific replay and downstream filtering were not completed. It is not a downstream training/filtering result.
...
| v3.6 process-annotation locked validation report | `outputs/real_task_v3_6_prm800k_hash/locked_validation_report.json` | Positive real process-annotation evidence: 4,417 samples, 34,219 labeled artifacts, `w_struct` Spearman 0.611, raw local utility Spearman -0.077, Holm pass. |
| v3.6 process-annotation decision report | `outputs/real_task_v3_6_prm800k_hash/decision_report.json` | Claim permission artifact for process-annotation consistency only. |
| v3.7 process-annotation overlap audit | `outputs/real_task_v3_7_prm_baseline_comparison/training_overlap_audit.json` | Bidirectional dataset-contamination audit used to restrict baseline wording to in-distribution comparison with acknowledged overlap risk. |
...
| v3.8 frozen PRM decision report | `outputs/real_task_v3_8_prm_locked_scoring/decision_report.json` | Claim permission artifact for `M_BASELINE_COMPARISON_CONTEXT_ONLY`; broad baseline comparison, PRM training, replay validation, and claims beyond process-annotation data are not allowed. |

Scope note: the process-annotation dataset provides positive real-data evidence for annotation-order consistency and in-distribution frozen PRM baseline context. GSM8K/HotpotQA task-specific replay and downstream filtering were not completed and do not provide additional validation evidence. These files do not add downstream filtering support, downstream training support, replay-pass evidence, mechanism-recovery claims, or any stronger readiness status.
