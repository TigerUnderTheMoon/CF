...
| CAV38-004 | REMEDIATED | reproducibility / package consistency | `paper/kbs_submission/main.tex`; `paper/kbs_submission/main.pdf` | Data/code availability previously described GSM8K and HotpotQA downstream filtering data as part of the current protocol. | Replaced with v3.6/v3.8 PRM800K artifacts as current real-data evidence and replay/filtering artifacts as failed or blocked provenance. |
...
> The current positive real-data evidence is limited to PRM800K step-label ranking and an overlap-limited frozen PRM baseline context. GSM8K/HotpotQA replay and downstream filtering/training remain blocked or failed validation routes and are reported only as provenance or future validation targets.

## Reviewer Risk Register

| ID | Priority | Category | Reviewer concern | Evidence | Severity | Probability | Fix effort | Recommended action |
|---|---|---|---|---|---|---|---|---|
| RSK-001 | remediated | empirical evidence | Reviewer could say the paper claims positive GSM8K filtering even though current artifacts say downstream filtering failed. | CAV38-001 to CAV38-004 | fatal before fix | low after fix | completed | KBS source and PDF now report GSM8K/HotpotQA replay/filtering only as failed or blocked provenance. |
