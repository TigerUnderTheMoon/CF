...
| `C_S_FMA_V2_FRESH_HOLDOUT` | `structurally_calibrated_fma_v2` is a planned preregistered fresh-holdout route, not a completed validation. | `planned` | `paper/s_fma_v2_fresh_holdout_plan.md`; `configs/s_fma_v2_fresh_holdout.yaml`; `outputs/s_fma_v2_fresh_holdout/fresh_manifest.json`; `outputs/s_fma_v2_fresh_holdout/manifest_overlap_audit.json`; `outputs/s_fma_v2_fresh_holdout/manifest_overlap_audit.md`; `outputs/s_fma_v2_fresh_holdout/api_preflight_report.json` | current pilot is frozen as development failure audit; v2 is planned only; fresh manifest/audit is `MANIFEST_OVERLAP_CLEAN` after the empty-alias policy revision; API preflight-only reports `PREFLIGHT_FAIL_DRIFT` after 20 evaluated live API records; schema/tag/final-answer rates are all `1.0`; `system_fingerprint` is missing as disclosure-only metadata, but deterministic replay remains forbidden; no full generation; no 400 fresh traces; no v2 scoring; no replay; no PRM claim yet; current status remains `PILOT_BLOCKED` | `same_pilot_pass_assertion`; `current_trace_v2_validation_assertion`; `same_pilot_tuning_validation_assertion`; `single_task_global_unlock_assertion` |
