...
v2_1_route: `s_FMA_v2.1` planned-only; regenerated package clean for current prompt hash; approved API_PREFLIGHT_ONLY rerun now parses but remains drift-failed; approved transport canary passed as diagnostic-only extraction evidence
...
v2_1_next_allowed_step: only generate a v2.1 stochastic smoke approval request if route planning is clean; this is request-only and cannot automatically run smoke; current report field `v2_1_smoke_approval_request_allowed` remains `false`
...
- `s_FMA_v2.1` is a planned evidence-target revision with a regenerated clean manifest/contract package and a failed approved API_PREFLIGHT_ONLY diagnostic. The failed preflight report is now `PREFLIGHT_FAIL_DRIFT` with valid traces and successful parsing, but it is still not `API_PREFLIGHT_READY`; the drift failure audit blocks deterministic replay and leaves stochastic repeated replay as planning-only. The next allowed step is only generating a v2.1 stochastic smoke approval request if route planning is clean. No smoke run, replay, v2.1 scoring, validation, task/global pass, deterministic replay claim, or PRM claim exists.
