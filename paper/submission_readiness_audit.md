...
v2_2_route: `s_FMA_v2.2` manifest clean plus prompt lock, drift-failed API preflight, and request-only stochastic smoke approval package; new route after failed v2.1 full validation; no smoke, replay, scoring, validation/pass claim, deterministic replay claim, top-tier-ready claim, or PRM/filtering authorized
...
v2_2_stochastic_smoke_approval_request: `outputs/s_fma_v2_2_fresh_holdout/stochastic_smoke_approval_request.md` and `.json`; `REQUEST_ONLY_NOT_APPROVED`; scope `V2_2_STOCHASTIC_SMOKE_ONLY`; 20 total records, GSM8K 10 plus HotpotQA 10; 3 stochastic repeats per eligible span; max 140 API requests; recommended USD 8 ceiling; no smoke execution performed
...
| s_FMA_v2.2 request-only route | manifest clean; contract clean; prompt locked; API preflight drift-failed; stochastic smoke request not approved; no validation run | `outputs/s_fma_v2_2_fresh_holdout/fresh_manifest.json` has 400 rows, 200 GSM8K and 200 HotpotQA; `outputs/s_fma_v2_2_fresh_holdout/manifest_overlap_audit.json` reports `MANIFEST_OVERLAP_CLEAN` with zero selected overlap on all six hard keys; `outputs/s_fma_v2_2_fresh_holdout/v2_2_contract_audit.json` reports `V2_2_CONTRACT_CLEAN` and prompt hash `prompt-sha256:8f9498811a4534f390d4f0a0ea648aa3701565e05c89174b72f2b8ba2191ae52`; the current `outputs/s_fma_v2_2_fresh_holdout/api_preflight_report.json` has JSON/schema/tag/final-answer success and non-empty `raw_output` all `1.0`, but remains failed with `status: PREFLIGHT_FAIL_DRIFT`, failure codes `PREFLIGHT_FAIL_DRIFT` and `PREFLIGHT_FAIL_METADATA`, and disclosure-only missing `fallback_model`/`system_fingerprint` metadata; `outputs/s_fma_v2_2_fresh_holdout/stochastic_smoke_approval_request.json` is `REQUEST_ONLY_NOT_APPROVED`, scoped to `V2_2_STOCHASTIC_SMOKE_ONLY`, 20 records, 3 stochastic repeats per eligible span, max 140 API requests, and recommended USD 8 ceiling. The route remains a new route after failed v2.1 full validation, addresses GSM8K sparse binary Delta-U with repeated numeric-success probability, keeps HotpotQA normalized token F1 with explicit alias/normalization policy, and requires bounded audited schema/transport repair plus bootstrap uncertainty reporting. It cannot tune thresholds, select rows, fit weights, or upgrade claims from v2.1 full-validation artifacts, and it does not authorize smoke, replay, scoring, task/global gate satisfaction, validation/pass claims, deterministic replay claims, top-tier-ready claims, or PRM/filtering. |
...
- `V2_2_PREFLIGHT_DRIFT_DISCLOSED_STOCHASTIC_SMOKE_REQUEST_ONLY`: v2.2 now has a clean manifest plus prompt-locked package, but the current API preflight is still failed. JSON/schema/tag/final-answer success and non-empty `raw_output` are all `1.0`, while `status` remains `PREFLIGHT_FAIL_DRIFT` with failure codes `PREFLIGHT_FAIL_DRIFT` and `PREFLIGHT_FAIL_METADATA`; missing `fallback_model` and `system_fingerprint` are disclosure-only provider metadata. The stochastic smoke request has `approval_status: REQUEST_ONLY_NOT_APPROVED`, scope `V2_2_STOCHASTIC_SMOKE_ONLY`, 20 total records, 3 stochastic repeats per eligible span, max 140 API requests, and recommended USD 8 ceiling. This is not smoke execution, replay, scoring, validation evidence, task/global gate satisfaction, deterministic replay support, top-tier-ready evidence, or PRM/filtering evidence. The drift failure remains a deterministic-route blocker. v2.1 full-validation artifacts were used only as failed provenance and overlap-exclusion sources, not to tune thresholds, select rows, fit weights, or upgrade claims.
...
- `s_FMA_v2.2` is prompt-locked with a drift-failed API preflight and a request-only stochastic smoke approval package. It has a 400-row fresh manifest, clean non-overlap audit, clean contract audit, schema/tag/final-answer/raw-output gates at `1.0`, `PREFLIGHT_FAIL_DRIFT`, disclosure-only missing provider metadata, and `REQUEST_ONLY_NOT_APPROVED` stochastic smoke request files. It has no smoke, replay, Delta-U, scoring, rank-signal gate evaluation, task/global gate, validation/pass claim, deterministic replay claim, top-tier-ready claim, or PRM/filtering artifacts.
- PRM/filtering improvement remains future validation, not a completed result.

Do not mark the manuscript as top-tier ready until primary real-task rank signal/API drift gates, v2.1 full-validation gates, and the v2.2 preflight/drift plus separately approved stochastic-route blockers are resolved under their own approved scopes.
