...
v2_2_route: `s_FMA_v2.2` planned preregistration only; new route after failed v2.1 full validation; no API, manifest, replay, scoring, or PRM/filtering authorized
...
| s_FMA_v2.2 preregistration | planned only | `paper/s_fma_v2_2_preregistration_plan.md`, `configs/s_fma_v2_2_fresh_holdout.yaml`, and `paper/v2_1_to_v2_2_transition_audit.md` define a new route after failed v2.1 full validation. The route addresses GSM8K sparse binary Delta-U with repeated numeric-success probability, keeps HotpotQA normalized token F1 with explicit alias/normalization policy, requires bounded audited schema/transport repair accounting, and requires Spearman/Kendall/NDCG/top-k AUC bootstrap uncertainty reporting. It has no manifest or output artifacts yet, cannot tune thresholds from v2.1 full-validation artifacts, and does not authorize API, replay, scoring, task/global gate satisfaction, or PRM/filtering. |
...
- `S_FMA_V2_2_PREREGISTRATION_ONLY`: v2.2 is a new planned route after the failed v2.1 full validation. It must use fresh non-overlapping data or a new preregistered split and cannot use v2.1 full-validation artifacts to tune thresholds, select rows, fit weights, or upgrade claims.
...
The v2.2 preregistration is the separate route option. It changes the primary utility target before any new execution, requires a fresh non-overlapping holdout or new preregistered split, and locks schema/transport repair plus uncertainty reporting policy before future manifest/API work. The v2.1 full artifact remains failed provenance and cannot supply v2.2 thresholds.
...
- `s_FMA_v2.2` is preregistered only. It has no manifest, traces, replay, Delta-U, scoring, rank-signal, task/global gate, or PRM/filtering artifacts yet.
- PRM/filtering improvement remains future validation, not a completed result.

Do not mark the manuscript as top-tier ready until primary real-task rank signal/API drift gates and v2.1 full-validation pass gates are resolved.
