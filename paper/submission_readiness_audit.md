...
real_task_v3_route: planned preregistration only; hard budget caps, dense-target reliability gates, low-complexity `w_struct`, explicit baseline fairness ledger, and locked-validation decision tree are specified, but no API calls, manifest generation, replay, scoring, locked validation, downstream validation, or claim upgrade is authorized
real_task_v3_plan: `paper/real_task_v3_preregistration_plan.md`
real_task_v3_config: `configs/real_task_v3_validation.yaml`
real_task_v3_output_root: `outputs/real_task_v3/`; currently placeholder only
...
| real_task_v3 preregistered route | planned only; no execution authorized | `paper/real_task_v3_preregistration_plan.md` and `configs/real_task_v3_validation.yaml` define a new planned route with locked `1000+1000` validation scale, hard caps, dense-target reliability checks, low-complexity `w_struct`, baseline fairness requirements, and a one-shot locked decision tree. `outputs/real_task_v3/` is placeholder-only. No API call, manifest generation, replay, scoring, validation pass, deterministic replay, submission-upgrade, or PRM/filtering improvement claim is authorized. |
...
- `REAL_TASK_V3_PREREGISTRATION_ONLY`: v3 is a new planned route, not an executed validation. The preregistration config sets `api_execution_allowed: false`, `manifest_generation_authorized: false`, `replay_authorized: false`, `scoring_authorized: false`, and `prm_filtering_authorized: false`; current status remains `PILOT_BLOCKED`.
...
- `real_task_v3` is planned preregistration only. It defines the next possible validation route but currently has no smoke, dev calibration, locked validation, downstream validation, pass claim, deterministic replay claim, or PRM/filtering improvement artifact.
