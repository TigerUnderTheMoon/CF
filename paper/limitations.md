...
Downstream process-supervision evidence is negative under the only current mini-validation. The v2.1 downstream filtering mini-validation completed 20/20 valid paired samples but failed its filtering-signal gate, with pooled mean advantage -0.05. The manuscript can discuss structurally calibrated FMA as a candidate design constraint, but it cannot report PRM/filtering improvement, superiority, robustness, or generalization.

The real-task pilot remains guarded. The current readiness audit reports `PILOT_BLOCKED`, and API preflight still reports `PREFLIGHT_FAIL_DRIFT`. Real-task traces generated under the nondeterministic protocol can support pilot debugging and boundary analysis, but they should not be treated as validation evidence.
