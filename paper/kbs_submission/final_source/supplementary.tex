...
\begin{table}[ht]
\caption{Full MuSiQue KBS-style knowledge-audit prioritization results on the locked split (1,415 samples, 12,715 steps).}
...
The support decision is based on the best SC-FMA method versus the strongest non-SC-FMA control. SC-FMA QP reaches NDCG@25\% of 1.0000; graph centrality, the best control, reaches 0.8807. The resulting $\Delta$NDCG@25\% is 0.1193, with bootstrap 95\% CI [0.1187, 0.1199], exceeding the required threshold of 0.05. This supports KBS-style audit prioritization on constructed MuSiQue evidence-chain traces only; it is not production KBS deployment validation.
...
\item \textbf{MuSiQue audit route.} The KBS-style MuSiQue audit route is reproduced with \texttt{python scripts/run\_knowledge\_audit\_eval.py --hf-dataset dgslibisey/MuSiQue --source-format musique --hf-split train --max-records 2000 --shuffle-seed 42 --bootstrap-samples 1000 --bootstrap-seed 42}. It writes \texttt{outputs/kbs\_real\_audit\_v1/knowledge\_audit\_report.json}, \texttt{knowledge\_audit\_summary.md}, \texttt{knowledge\_audit\_traces.jsonl}, and \texttt{dataset\_manifest.json}; the run uses zero API calls.
\item \textbf{Output artifacts.} All raw results (CIU vectors, necessity scores, redundancy matrices, calibrated weights, ranking metrics) are saved as JSONL or JSON artifacts in the \texttt{outputs/} directory. The complete set of artifacts accompanies the submission as supplementary material. The MuSiQue route is archived under \texttt{outputs/kbs\_real\_audit\_v1} and is labeled \texttt{kbs\_style\_audit\_prioritization\_evidence\_only}.
\end{enumerate}
