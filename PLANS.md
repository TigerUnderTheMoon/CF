...
The approved v2.1 API_PREFLIGHT_ONLY rerun used the regenerated prompt-locked package and rewrote only the allowed preflight outputs. It remains `PREFLIGHT_FAIL_DRIFT` after 20 records and 23 API attempts, with cost `0.86245`, JSON/schema/tag/final-answer success `1.0`, 20 valid trace rows, and 23/23 non-empty `raw_output` attempts; the current preflight failure is drift plus missing metadata, not empty extracted output. The latest bounded v2.1 stochastic smoke rerun wrote only the allowed smoke artifacts under `outputs/s_fma_v2_1_fresh_holdout/` and reports `V2_1_STOCHASTIC_SMOKE_FEASIBLE_FOR_PILOT_REQUEST`: JSON/schema/tag/final-answer success rates are `1.0`, replay success rate is `1.0`, and nonzero Delta-U counts are 20 pooled, 7 GSM8K, and 13 HotpotQA. The recomputed v2.1 pilot stochastic report reports `V2_1_PILOT_STOCHASTIC_PASS`, but this remains pilot stochastic gate evidence only. The later full stochastic validation failed its preregistered gates despite positive rank signal: quality rates were `0.9971181556195965` rather than required `1.0`, and GSM8K nonzero Delta-U was `16 < 20`. The strict engineering retry failed as well: it used 119 incremental retry API calls, produced an effective report with 2794 API attempts, retained `GLOBAL_pass: false`, and hard-stopped with `transport_unresolved_and_gsm8k_sparse_signal_below_preregistered_threshold`. Strict v2.1 full validation is therefore abandoned, not rescued. No deterministic replay claim, full validation pass claim, submission-ready claim, top-tier-ready claim, gate-relaxation claim, or PRM/filtering claim is allowed from these results.

The one-shot v2.1 downstream filtering mini-validation was preregistered separately from full validation and executed once under a USD `5` ceiling and 60-request cap. It finished at 40 API calls and USD `1.629725`, with 20/20 valid paired samples, but failed the filtering-signal gate: pooled mean advantage was `-0.05`, GSM8K `-0.2`, and HotpotQA `0.1`. This route is abandoned as a failed mini diagnostic, not retried or scaled. It does not unlock `DOWNSTREAM_PRM_FILTERING_VALIDATION_PASS`, PRM/filtering claims, v2.4, or any readiness upgrade.

The planned v2.2 route changes the evidence target before any full validation execution: GSM8K uses repeated numeric-success probability rather than a single sparse binary Delta-U row, HotpotQA keeps normalized token F1 with explicit alias handling, schema/transport failures require bounded audited repair accounting, and rank signal reporting must include Spearman, Kendall, NDCG, top-k AUC, and bootstrap uncertainty. The v2.2 fresh manifest is locked under `outputs/s_fma_v2_2_fresh_holdout/` with a clean non-overlap and contract audit, and the generation/replay prompts are locked to `prompt-sha256:8f9498811a4534f390d4f0a0ea648aa3701565e05c89174b72f2b8ba2191ae52`. The current API preflight is still failed: JSON/schema/tag/final-answer success and non-empty `raw_output` are all `1.0`, but `drift_status` remains `PREFLIGHT_FAIL_DRIFT`; `fallback_model` and `system_fingerprint` remain disclosure-only missing metadata. The current smoke checkpoint consumed 142 requests and USD `2.00082`, restored 20/20 valid originals, completed 120/120 replay attempts, and reached JSON/schema/tag/final-answer/raw-output success `1.0`, but fails sparse-signal gates with nonzero Delta-U counts of 5 pooled, 0 GSM8K, and 5 HotpotQA. Pilot validation, route-pass wording, and PRM/filtering remain blocked.
...
- Fresh manifest/audit is clean for v2, but the first 20-row stochastic smoke failed original generation and the bounded rerun failed with sparse Delta-U signal; full fresh traces, v2 scoring, replay validation, and rank-signal validation have not run. v2.1 has separate package materialization, a passed transport-only canary, an approved API_PREFLIGHT_ONLY rerun that now parses successfully but remains `PREFLIGHT_FAIL_DRIFT` with missing disclosure metadata, a bounded stochastic smoke rerun that was feasible for a pilot-budget request, and a recomputed pilot stochastic artifact whose task/global pilot gates pass. The later v2.1 full stochastic validation and strict engineering retry are failed provenance, not a pass: rank signal is positive, but preregistered quality and GSM8K sparse-signal gates failed, and strict v2.1 full validation is now abandoned as non-viable under the current contract. v2.2 is now preregistered, manifest-locked, prompt-locked, and current API preflight still reports `PREFLIGHT_FAIL_DRIFT` even though schema/tag/final-answer/raw-output success rates are `1.0`; missing provider metadata is disclosure-only. Its current stochastic smoke completed after approved repair/resume but fails sparse-signal gates, especially GSM8K nonzero Delta-U `0`; it does not authorize pilot validation, scoring beyond smoke diagnostics, validation claims, route-pass wording, or PRM/filtering.
- PRM/filtering is a required top-tier downstream validation, not a completed result; the completed v2.1 mini downstream filtering diagnostic failed and is abandoned.
