...
| s_FMA_v2.2 preregistration | Planned / preregistration only | New route after failed v2.1 full validation; documented in `paper/s_fma_v2_2_preregistration_plan.md`, `configs/s_fma_v2_2_fresh_holdout.yaml`, and `paper/v2_1_to_v2_2_transition_audit.md`; requires a fresh non-overlapping holdout or new preregistered split; uses a repeated numeric-success probability utility target for GSM8K, HotpotQA normalized token F1 with explicit alias/normalization policy, bounded audited schema/transport repair policy, and Spearman/Kendall/NDCG/top-k AUC bootstrap uncertainty reporting; no v2.1 full-validation threshold tuning; no API, manifest, replay, scoring, task/global gate satisfaction, or PRM/filtering |
...
The v2.2 preregistration is a separate route, not a rescue of the v2.1 full artifact. It may use v2.1 only as failure provenance and design motivation. It must not use the v2.1 full-validation rows, thresholds, rank signal, or failure counts to tune v2.2 gates, choose rows, fit weights, or upgrade claims.
