...
| s_FMA_v2.1 evidence-target revision | Full stochastic validation failed / pilot stochastic pass only | HotpotQA `normalized_token_f1` target, GSM8K `question_difficulty_proxy` selection, and span-diversity prompt policy materialized; schema-fix non-API package regeneration produced `outputs/s_fma_v2_1_fresh_holdout/fresh_manifest.json` with 400 rows, zero selected overlap, `V2_1_CONTRACT_CLEAN`, and request-only API_PREFLIGHT_ONLY approval files locked to prompt hash `prompt-sha256:e5ac816bc586ee33a2800fbd0c373523154e0c4eeef74cdd349fa70271054a4b`; the approved API_PREFLIGHT_ONLY rerun remains `PREFLIGHT_FAIL_DRIFT`; the bounded stochastic smoke rerun was feasible for a pilot-budget request; the recomputed pilot stochastic report passed pilot gates only; the full stochastic validation artifact reports `V2_1_FULL_STOCHASTIC_FAIL_SCHEMA_OR_TAGS`, positive pooled/GSM8K/HotpotQA rank signal, `TASK_SPECIFIC_pass: false`, and `GLOBAL_pass: false`; failure sources are 8 timeout/connection attempts lowering JSON/schema/tag/final-answer rates to `0.9971181556195965` and GSM8K nonzero Delta-U `16 < 20`; the failure audit is `outputs/s_fma_v2_1_fresh_holdout/v2_1_full_validation_failure_audit.json` and `.md`; no deterministic replay, top-tier-ready, or PRM/filtering claim is allowed; status remains `PILOT_BLOCKED` |
| s_FMA_v2.2 request-only route | Planned / prompt-locked / API preflight drift-failed / stochastic smoke request-only | New route after failed v2.1 full validation; documented in `paper/s_fma_v2_2_preregistration_plan.md`, `configs/s_fma_v2_2_fresh_holdout.yaml`, and `paper/v2_1_to_v2_2_transition_audit.md`; artifacts under `outputs/s_fma_v2_2_fresh_holdout/` contain 400 rows, 200 GSM8K and 200 HotpotQA, `MANIFEST_OVERLAP_CLEAN`, zero selected overlap on all six hard keys, and `V2_2_CONTRACT_CLEAN`; prompt files `prompts/s_fma_v2_2_reflection_generation.txt` and `prompts/s_fma_v2_2_replay.txt` are locked to `prompt-sha256:8f9498811a4534f390d4f0a0ea648aa3701565e05c89174b72f2b8ba2191ae52`; the current v2.2 API preflight report has JSON/schema/tag/final-answer success and non-empty `raw_output` all `1.0`, but remains failed with `status: PREFLIGHT_FAIL_DRIFT`, failure codes `PREFLIGHT_FAIL_DRIFT` and `PREFLIGHT_FAIL_METADATA`, and disclosure-only missing `fallback_model`/`system_fingerprint` metadata; `stochastic_smoke_approval_request.{json,md}` is `REQUEST_ONLY_NOT_APPROVED`, scoped to `V2_2_STOCHASTIC_SMOKE_ONLY`, 20 records, 3 stochastic repeats per eligible span, max 140 API requests, and recommended USD 8 ceiling; no v2.1 full-validation threshold tuning, weight fitting, or row selection; no smoke approval, replay, scoring, task/global gate satisfaction, validation/pass claim, deterministic replay claim, top-tier-ready claim, or PRM/filtering |
| PRM/filtering validation | Required future work | Structurally calibrated supervision/filtering experiment |

The v2.1 pilot transport failure audit and single retry package remain provenance for the corrected pilot route. The former request-only full validation package is retained as approval provenance, while the current execution artifacts are frozen as failed full-validation provenance under `outputs/s_fma_v2_1_fresh_holdout/v2_1_full_stochastic_*` plus `v2_1_full_validation_failure_audit.*`. The full artifact does not authorize deterministic replay language, top-tier-ready claim, status upgrade, or PRM/filtering.

The v2.2 preregistration is a separate route, not a rescue of the v2.1 full artifact. It may use v2.1 only as failure provenance and design motivation. It must not use the v2.1 full-validation rows, thresholds, rank signal, or failure counts to tune v2.2 gates, choose rows, fit weights, or upgrade claims. The current v2.2 preflight is still drift-failed after clean schema/tag/final-answer/raw-output gates: `PREFLIGHT_FAIL_DRIFT` blocks deterministic replay wording, while the stochastic route is only a drift-disclosed repeated-replay smoke request with `REQUEST_ONLY_NOT_APPROVED`.
