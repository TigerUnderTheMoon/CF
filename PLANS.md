...
| s_FMA_v2 fresh holdout | Planned / preflight drift-failed / smoke generation-failed | Fresh manifest/audit clean; guarded API preflight-only reports `PREFLIGHT_FAIL_DRIFT`; approved 20-row stochastic smoke reports `STOCHASTIC_SMOKE_FAIL_GENERATION`; generation failure audit added; no full generation, no v2 scoring, no task/global pass |
...
- Fresh manifest/audit is clean, but the 20-row stochastic smoke failed original generation; full fresh traces, v2 scoring, replay validation, and rank-signal validation have not run. Fresh holdout validation is still required for any `s_FMA_v2` real-task rank-signal claim.
