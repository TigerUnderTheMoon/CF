...
| s_FMA_v2.1 evidence-target revision | Planned / API preflight drift-failed / pilot stochastic blocked by one transport failure | HotpotQA `normalized_token_f1` target, GSM8K `question_difficulty_proxy` selection, and span-diversity prompt policy materialized; schema-fix non-API package regeneration produced `outputs/s_fma_v2_1_fresh_holdout/fresh_manifest.json` with 400 rows, zero selected overlap, `V2_1_CONTRACT_CLEAN`, and request-only API_PREFLIGHT_ONLY approval files locked to prompt hash `prompt-sha256:e5ac816bc586ee33a2800fbd0c373523154e0c4eeef74cdd349fa70271054a4b`; the approved API_PREFLIGHT_ONLY rerun remains `PREFLIGHT_FAIL_DRIFT`; the bounded stochastic smoke rerun was feasible for a pilot-budget request; the current pilot stochastic report is frozen as blocked provenance after 700 API requests, cost USD `28.04808`, 100 valid original traces, 599/600 replay success, JSON/schema/tag/final-answer success `0.9985714285714286`, nonzero Delta-U counts of 96 pooled, 42 GSM8K, and 54 HotpotQA, pooled/GSM8K/HotpotQA Spearman CIs above zero, but one failed replay transport row `gsm8k-00357` span `1` repeat `2` with `api_error:APIConnectionError:Connection error.`; current `TASK_SPECIFIC` and `GLOBAL` pass remain false, status remains `PILOT_BLOCKED`; no v2.1 validation, task/global pass, full validation, deterministic replay claim, or PRM claim; next allowed step is only user review of the request-only `V2_1_PILOT_SINGLE_TRANSPORT_RETRY_ONLY` package |
| PRM/filtering validation | Required future work | Structurally calibrated supervision/filtering experiment |

The v2.1 pilot transport failure audit is stored at `outputs/s_fma_v2_1_fresh_holdout/v2_1_pilot_transport_failure_audit.md` and `.json`. The single retry approval package is stored at `outputs/s_fma_v2_1_fresh_holdout/v2_1_pilot_single_retry_approval_request.md` and `.json`; it is `REQUEST_ONLY_NOT_APPROVED`, caps any future approved retry at 3 API requests and USD 1, permits only `gsm8k-00357` / `gsm8k-test-00357` / span `1` / repeat `2`, and authorizes no API call, retry, original regeneration, other replay attempt, full validation, pass claim, or PRM/filtering without separate explicit user approval.
...
- Fresh manifest/audit is clean for v2, but the first 20-row stochastic smoke failed original generation and the bounded rerun failed with sparse Delta-U signal; full fresh traces, v2 scoring, replay validation, and rank-signal validation have not run. v2.1 has separate package materialization, a passed transport-only canary, an approved API_PREFLIGHT_ONLY rerun that now parses successfully but remains `PREFLIGHT_FAIL_DRIFT` with missing disclosure metadata, a bounded stochastic smoke rerun that was feasible for a pilot-budget request, and a current pilot stochastic artifact blocked only by one replay transport/APIConnectionError row. The v2.1 pilot has nonzero Delta-U in both tasks and positive pilot Spearman CIs, but it remains blocked provenance because JSON/schema/tag/final-answer success is `0.9985714285714286` rather than the preregistered perfect hard stop. Fresh holdout validation is still required for any `s_FMA_v2` or v2.1 task/global pass claim.
...
The current failure audit is frozen. The next top-tier route is the planned `s_FMA_v2` or revised v2.1 fresh holdout, not same-pilot retuning. The v2 scorer is frozen by formula hash `sha256:6971b23562be690e5fd58dc4dfbbcf972d2137c719b1b68a440d9ec4a216b628`. `outputs/s_fma_v2_fresh_holdout/manifest_overlap_audit.json` is currently `MANIFEST_OVERLAP_CLEAN`; `outputs/s_fma_v2_fresh_holdout/api_preflight_report.json` is `PREFLIGHT_FAIL_DRIFT` after a guarded live API preflight-only run with 20 evaluated records. `system_fingerprint` was missing for all 20 evaluated records and is disclosed separately from schema/tag/final-answer success. `outputs/s_fma_v2_fresh_holdout/stochastic_smoke_report.json` is `STOCHASTIC_SMOKE_FAIL_SPARSE_SIGNAL`: the bounded rerun used 20 smoke samples, produced 60/60 successful replay results, spent `3.14542` USD within the `5` USD ceiling, but had `nonzero_delta_rows: 0` and `next_allowed_step: STOP_OR_REVISE_EVIDENCE_TARGET`. `outputs/s_fma_v2_1_fresh_holdout/manifest_overlap_audit.json` is `MANIFEST_OVERLAP_CLEAN`, `outputs/s_fma_v2_1_fresh_holdout/v2_1_contract_audit.json` is `V2_1_CONTRACT_CLEAN`, and the approved `outputs/s_fma_v2_1_fresh_holdout/api_preflight_report.json` rerun is `PREFLIGHT_FAIL_DRIFT` after 20 records and 23 API attempts, with cost `0.86245`, 20 valid trace rows, JSON/schema/tag/final-answer success `1.0`, and 23/23 non-empty `raw_output` attempts. `outputs/s_fma_v2_1_fresh_holdout/api_preflight_drift_failure_audit.json` records that deterministic replay is blocked by drift. `outputs/s_fma_v2_1_fresh_holdout/stochastic_smoke_report.json` is `V2_1_STOCHASTIC_SMOKE_FEASIBLE_FOR_PILOT_REQUEST`: the latest bounded smoke rerun used 140 API attempts, spent `6.11314` USD within the `8` USD ceiling, reached JSON/schema/tag/final-answer success `1.0`, replay success rate `1.0`, 40 Delta-U rows, and nonzero Delta-U counts of 20 pooled, 7 GSM8K, and 13 HotpotQA. `outputs/s_fma_v2_1_fresh_holdout/v2_1_pilot_stochastic_report.json` is blocked provenance after 700 API requests and USD `28.04808`, with 100 valid original traces, 599/600 replay success, JSON/schema/tag/final-answer success `0.9985714285714286`, nonzero Delta-U counts of 96 pooled, 42 GSM8K, and 54 HotpotQA, and positive pilot Spearman CIs; the only hard-stop defect is the single failed replay transport row `gsm8k-00357` / `gsm8k-test-00357` / span `1` / repeat `2`, `api_error:APIConnectionError:Connection error.`. The new request-only single retry approval package is `outputs/s_fma_v2_1_fresh_holdout/v2_1_pilot_single_retry_approval_request.json` and `.md`, with scope `V2_1_PILOT_SINGLE_TRANSPORT_RETRY_ONLY`, USD 1 budget ceiling, 3-request cap, and permission for only that failed replay attempt after explicit approval. No full generation, no 400 fresh validation, no v2/v2.1 validation, no task/global pass, no deterministic replay claim, no full validation, no PRM claim yet, and current status remains `PILOT_BLOCKED`. PRM/filtering validation remains blocked until a global fresh-holdout validation gate or a separate downstream validation gate.
