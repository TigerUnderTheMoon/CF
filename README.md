...
The current state is not a completed top-tier PRM/filtering result. Real-task replay is still pilot evidence, readiness is `PILOT_BLOCKED`, and attribution-aware PRM/filtering remains a required downstream validation experiment. A one-shot v2.1 downstream filtering mini-validation has now run and failed its preregistered filtering-signal gate; it is failed diagnostic evidence, not a PRM/filtering success.
...
Until that final layer exists as passing comparative artifacts, PRM/filtering claims remain future validation requirements rather than completed evidence. The failed v2.1 mini filtering diagnostic does not satisfy that layer.
...
The v2.2 preregistration files add a planned fresh route under `paper/s_fma_v2_2_preregistration_plan.md` and `configs/s_fma_v2_2_fresh_holdout.yaml`. The package under `outputs/s_fma_v2_2_fresh_holdout/` contains the manifest/contract artifacts, current failed `api_preflight_report.json`, preflight failure audit, smoke checkpoint artifacts, and sparse-signal failure audit. It also locks `prompts/s_fma_v2_2_reflection_generation.txt` and `prompts/s_fma_v2_2_replay.txt` to prompt hash `prompt-sha256:8f9498811a4534f390d4f0a0ea648aa3701565e05c89174b72f2b8ba2191ae52`. The current preflight has JSON/schema/tag/final-answer success and non-empty `raw_output` at `1.0`, but `status` and `drift_status` remain `PREFLIGHT_FAIL_DRIFT`; failure codes are `PREFLIGHT_FAIL_DRIFT` and `PREFLIGHT_FAIL_METADATA`, with `fallback_model` and `system_fingerprint` missing as disclosure-only metadata. The current smoke completed after the approved provider retry and replay resume, but fails sparse-signal gates: 20/20 valid originals, 120/120 replay success, 40 Delta-U rows, nonzero Delta-U counts of 5 pooled, 0 GSM8K, and 5 HotpotQA. It does not authorize pilot validation, scoring beyond smoke diagnostics, validation pass wording, task/global gate satisfaction, route-pass wording, submission-upgrade wording, or PRM/filtering evidence.
