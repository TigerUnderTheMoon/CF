...
The recomputed v2.1 pilot stochastic artifact at `outputs/s_fma_v2_1_fresh_holdout/v2_1_pilot_stochastic_report.json` now reports `V2_1_PILOT_STOCHASTIC_PASS` after the single transport retry: 700 effective API requests, cost USD `28.06931`, 100 valid original traces, 600/600 replay success, JSON/schema/tag/final-answer success rates all `1.0`, nonzero Delta-U counts of 96 pooled, 42 GSM8K, and 54 HotpotQA, and positive pooled/GSM8K/HotpotQA Spearman CIs. Its `TASK_SPECIFIC_pass` and `GLOBAL_pass` fields are true only at the pilot stochastic gate. It is not full validation, deterministic replay evidence, top-tier-ready evidence, or a PRM/filtering result; current status remains `PILOT_BLOCKED`.
...
The planned `s_FMA_v2.2` route is now preregistered in `paper/s_fma_v2_2_preregistration_plan.md`, `configs/s_fma_v2_2_fresh_holdout.yaml`, and `paper/v2_1_to_v2_2_transition_audit.md`, with prompt-locked artifacts under `outputs/s_fma_v2_2_fresh_holdout/`. It is a new route after the failed v2.1 full validation, not a reinterpretation of v2.1. v2.2 targets GSM8K binary Delta-U sparsity with a repeated numeric-success probability utility target, keeps HotpotQA on normalized token F1 with explicit alias/normalization policy, requires bounded and audited schema/transport repair rather than silent dropping, and requires Spearman/Kendall/NDCG/top-k AUC bootstrap uncertainty reporting. The package contains 400 manifest rows, 200 GSM8K and 200 HotpotQA, with `MANIFEST_OVERLAP_CLEAN`, zero selected overlap on all six hard keys, `V2_2_CONTRACT_CLEAN`, and prompt hash `prompt-sha256:8f9498811a4534f390d4f0a0ea648aa3701565e05c89174b72f2b8ba2191ae52` for `prompts/s_fma_v2_2_reflection_generation.txt` plus `prompts/s_fma_v2_2_replay.txt`. The current v2.2 API preflight artifact at `outputs/s_fma_v2_2_fresh_holdout/api_preflight_report.json` is failed, not a pass: JSON/schema/tag/final-answer success and non-empty `raw_output` are all `1.0`, but status remains `PREFLIGHT_FAIL_DRIFT` with failure codes `PREFLIGHT_FAIL_DRIFT` and `PREFLIGHT_FAIL_METADATA`; missing `fallback_model`/`system_fingerprint` is disclosure-only metadata. The stochastic smoke approval package is request-only at `outputs/s_fma_v2_2_fresh_holdout/stochastic_smoke_approval_request.{json,md}`, scoped to `V2_2_STOCHASTIC_SMOKE_ONLY`, 20 records, 3 stochastic repeats per eligible span, max 140 API requests, and a recommended USD 8 ceiling. v2.1 full-validation artifacts were used only as failed provenance and overlap-exclusion sources, not as tuning, weighting, threshold, or row-selection sources. Current status remains `PILOT_BLOCKED`; no smoke, replay, scoring, pilot/full validation, v2.2 pass claim, deterministic replay claim, top-tier-ready claim, or PRM/filtering route is authorized by the request-only stochastic-smoke package.
...
The v2.2 preregistration files add a planned fresh route under `paper/s_fma_v2_2_preregistration_plan.md` and `configs/s_fma_v2_2_fresh_holdout.yaml`. The package under `outputs/s_fma_v2_2_fresh_holdout/` contains `fresh_manifest.json`, `manifest_overlap_audit.{json,md}`, `v2_2_contract_audit.{json,md}`, the request-only `api_preflight_approval_request.{json,md}`, the current failed `api_preflight_report.json`, the failure audit, and the request-only stochastic smoke approval package. It also locks `prompts/s_fma_v2_2_reflection_generation.txt` and `prompts/s_fma_v2_2_replay.txt` to prompt hash `prompt-sha256:8f9498811a4534f390d4f0a0ea648aa3701565e05c89174b72f2b8ba2191ae52`. The current preflight has JSON/schema/tag/final-answer success and non-empty `raw_output` at `1.0`, but `status` and `drift_status` remain `PREFLIGHT_FAIL_DRIFT`; failure codes are `PREFLIGHT_FAIL_DRIFT` and `PREFLIGHT_FAIL_METADATA`, with `fallback_model` and `system_fingerprint` missing as disclosure-only metadata. The stochastic smoke request is `REQUEST_ONLY_NOT_APPROVED`, limited to `V2_2_STOCHASTIC_SMOKE_ONLY`, 20 total records, 3 stochastic repeats per eligible span, max 140 API requests, and a recommended USD 8 ceiling. This is not smoke approval, validation, replay, scoring, task/global gate satisfaction, deterministic replay support, top-tier-ready evidence, or PRM/filtering evidence.
