...
The current real-task pilot failed the primary rank-signal gate and is frozen as a development failure audit. `s_FMA_v2` remains a fresh-holdout validation route, but the guarded live API preflight-only run reports `PREFLIGHT_FAIL_DRIFT`, the first approved 20-row stochastic smoke reported `STOCHASTIC_SMOKE_FAIL_GENERATION`, and the bounded rerun reports `STOCHASTIC_SMOKE_FAIL_SPARSE_SIGNAL`. The smoke generation failure audit is stored at `outputs/s_fma_v2_fresh_holdout/stochastic_smoke_generation_failure_audit.md` and `.json`; the approved rerun consumed `3.14542` USD within the `5` USD ceiling, produced 60/60 successful replay results, but had `nonzero_delta_rows: 0`. The `s_FMA_v2.1` evidence-target revision has a bounded API preflight-only run at `outputs/s_fma_v2_1_fresh_holdout/api_preflight_report.json`: status `PREFLIGHT_FAIL_SCHEMA_OR_TAGS`, 20 records, 23 API attempts, cost `0.837825`, JSON/tag/final-answer success rates `1.0`, schema success rate `0.85`, and drift status `PREFLIGHT_FAIL_DRIFT`. The schema failure audit is stored at `outputs/s_fma_v2_1_fresh_holdout/api_preflight_schema_failure_audit.md` and `.json`; it identifies a `self_evaluation` versus `self-evaluation` enum mismatch, adds a local parser/prompt/config fix, and marks the old prompt-locked v2.1 package stale for future API. Current status remains `PILOT_BLOCKED`. No validation or pass claim is available; no smoke approval request, full generation, v2/v2.1 scoring, replay, task/global pass, deterministic replay claim, or PRM claim is allowed from these artifacts.
