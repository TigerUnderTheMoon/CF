...
| Open-source data ingestion | Planned (no files) | **Partially implemented (dead code).** `src/fma/data/` has loaders for PRM800K, ProcessBench, GSM8K CoT, but no operational script calls them. PRM800K pipeline (`run_scfma_variants_prm800k.py`) reads raw JSONL directly from GitHub URL, bypassing `fma.data`. ProcessBench loader is complete but pipeline-isolated (needs adapter to `RankingSample`). |
| Frozen PRM scoring | Planned (no files) | **Partially implemented.** `src/fma/prm/` wraps Qwen2.5-Math-PRM (verified working in v3.8 via custom `SkyworkPrefixRewardScorer`). Math-Shepherd registered with wrong HF ID (`MathShepherd/` org doesn't exist; correct is `peiyi9979/math-shepherd-mistral-7b-prm`) and incompatible architecture (`AutoModelForCausalLM` vs `AutoModelForTokenClassification`). RLVR-PRM registered with non-existent HF ID (`rlvr/rlvr-prm` returns 0 results). Neither Math-Shepherd nor RLVR-PRM can run with current `FrozenPRMScorer`. |
