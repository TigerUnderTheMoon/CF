...
from fma.pilot.api_client import APIResponse, BaseAPIClient, OpenAIClient
...
class OpenAIResponsesAdapter:
    ...
    def __init__(self, *, api_client: BaseAPIClient | None = None) -> None:
        self.api_client = api_client or OpenAIClient(model_name="gpt-5.5")
        self.openai_version = getattr(self.api_client, "openai_version", "api-client")
    ...
    def create_trace(
        ...
        *,
        ...
        _sync_client_runtime_controls(self.api_client, config)
        ...
        for retry_label, candidate in _request_candidates(request):
            ...
            try:
                response = self.api_client.chat_complete(
                    prompt=prompt,
                    model_name=model_name,
                    temperature=float(config.get("model", {}).get("temperature", 0.0)),
                    top_p=float(config.get("model", {}).get("top_p", 1.0)),
                    seed=int(config.get("experiment", {}).get("seed", 20260530)),
                    request_overrides=candidate,
                ...
                continue
            return _result_from_api_response(response, request_metadata=request_metadata)
        if last_error is not None:
            raise last_error
        raise RuntimeError("No OpenAI request candidate was attempted.")


def _sync_client_runtime_controls(api_client: BaseAPIClient, config: Mapping[str, Any]) -> None:
    pricing = config.get("pricing") if isinstance(config, Mapping) else None
    if isinstance(pricing, Mapping) and hasattr(api_client, "pricing"):
        setattr(api_client, "pricing", dict(pricing))
    budget = config.get("experiment", {}).get("user_approved_budget_usd") if isinstance(config, Mapping) else None
    tracker = getattr(api_client, "cost_tracker", None)
    if budget is not None and tracker is not None and getattr(tracker, "cost_ceiling_usd", None) is None:
        tracker.cost_ceiling_usd = float(budget)
...
def _result_from_api_response(response: APIResponse, *, request_metadata: dict[str, Any]) -> ApiCallResult:
    metadata = dict(response.metadata)
    response_metadata = metadata.get("request_metadata")
    if isinstance(response_metadata, Mapping):
        request_metadata = {**request_metadata, **dict(response_metadata)}
    return ApiCallResult(
        output_text=response.raw_output,
        model_name=response.model_name,
        system_fingerprint=_string_or_none(metadata.get("system_fingerprint")),
        usage=dict(response.usage),
        raw_response=getattr(response, "raw_response", None) or metadata.get("raw_response", response),
        request_metadata=request_metadata,
        response_id=_string_or_none(metadata.get("response_id")),
        output_extraction_diagnostics=dict(metadata.get("output_extraction_diagnostics") or {}),
    )
